...
background_model = None  # Initialize background model
alpha = 0.1  # Background update factor
...
def update_background(current_data, background, alpha):
    if background is None:
        ...
        return (1 - alpha) * background + alpha * current_data


def heatmap(frame):
    global first_slide_window, second_slide_window, background_model
    # データ取得
    sensordata = np.array(sensor.pixels)

    # Apply Gaussian filter
    filtered_data = gaussian_filter(sensordata, sigma=1)

    # Update background model
    background_model = update_background(filtered_data, background_model, alpha)

    # Compute foreground by subtracting background(去掉背景雜訊，凸顯人體位置)
    foreground = filtered_data - background_model
    ...
    # 消除背景雜訊，凸顯人體位置
    fore_y, fore_x = np.unravel_index(np.argmax(foreground), foreground.shape)
    axes[1].clear()
    axes[1].imshow(foreground, cmap="inferno", interpolation="bicubic")
    axes[1].scatter(fore_x, fore_y, color='white', s=120)  # 標記最熱點
    axes[1].set_title("Foreground Data")
    ...
    if len(first_slide_window) == first_window_size:
        ...
        axes[2].clear()
        axes[2].imshow(first_ma, cmap="inferno", interpolation="bicubic")
        axes[2].scatter(first_ma_x, first_ma_y, color='white', s=120)  # 標記最熱點
        axes[2].set_title("One Slide Moving Average")
    ...
    if len(second_slide_window) == second_window_size:
        ...
        axes[3].clear()
        axes[3].imshow(second_ma, cmap="inferno", interpolation="bicubic")
        axes[3].scatter(second_ma_x, second_ma_y, color='white', s=120)  # 標記最熱點
        axes[3].set_title("Two Slide Moving Average")
        axes[3].text(0.95, 0.95, f'Hotspot\nx: {second_ma_x}\ny: {second_ma_y}', 
                     horizontalalignment='right', verticalalignment='top', 
                     transform=axes[3].transAxes, color='white', fontsize=10)
    else:
        axes[3].clear()
        axes[3].imshow(sensordata, cmap="inferno", interpolation="bicubic")
        axes[3].set_title("Original Data")
